...
import org.apache.storm.report.TimeReportBolt;
 ...
 * 提交stormtopology任务
 * storm jar wordCount-2.0.0-SNAPSHOT.jar org.apache.storm.SentenceWordCountTimeTopology stormwordcount 2 1 1 1 0
 */
public class SentenceWordCountTimeTopology {
    private static final String SPOUT_TIMEREPORT_BOLT_ID = "spout-time-report";
    ...
    public static void main(String[] args) throws Exception {
        ...
        Integer timereportboltparallelism=Integer.valueOf(args[4]);
        long waitTimeNanos=Long.valueOf(args[5]);

        SentenceTimeSpout spout=new SentenceTimeSpout(waitTimeNanos);
        WordCountTimeTupleBolt wordCountTimeTupleBolt =new WordCountTimeTupleBolt();

        TimeReportBolt timeReportBolt =new TimeReportBolt();

        builder.setSpout(SENTENCE_SPOUT_ID,spout,spoutparallelism);
        builder.setBolt(COUNT_BOLT_ID, wordCountTimeTupleBolt,wordcountboltparallelism)
                .fieldsGrouping(SENTENCE_SPOUT_ID,WORDCOUNT_STREAM_ID,new Fields("word"));
        builder.setBolt(SPOUT_TIMEREPORT_BOLT_ID, timeReportBolt,timereportboltparallelism)
